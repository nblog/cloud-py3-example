...
from cloud_py3._common import (
    NOHTTPGET, EXTRACT, IS_64BIT, IS_ARM64, HTTPGET
)
...
class NETWORK:
    ...
    class interfaceCfg:
        ...
        def __init__(self, index: int, alias: str):
            ...
            output = ShellRunner.getoutput(f"netsh interface ipv4 show addresses name={self.index}")
    ...
    def __init__(self):
        output = re.findall( \
            "([0-9]+): (.*?)[\n\r]", 
            ShellRunner.getoutput("netsh interface ipv4 show ipaddresses"))
...
'''
    ...
    ''' https://github.com/google/vivsys/tree/master/vivsys '''

    try:
        from win32.lib import win32serviceutil
    except ImportError:
        os.environ.setdefault("PIP_INSTALL_PACKAGES", "pywin32")
        from cloud_py3.dynamic_pip import DynamicPip
